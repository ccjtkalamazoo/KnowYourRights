...
// questions or the ladder. The engine (and the map) both build on these.
//
...
// ---------------------------------------------------------------------------
// LifeIcon : inlined Lucide icons (MIT) for the lifelines.
// ---------------------------------------------------------------------------
...
//
// `shield` is kept even though the SHIELD lifeline was removed when lives
// landed. It costs four lines, and if a shield-shaped mechanic ever comes back
// the icon is already here and already matches the set. Nothing renders it
// today; rules.LIFELINE_KEYS is what decides which ones the shop shows.
export const LIFE_ICONS = {
  ...
  // shield: retired with the lifeline, kept as artwork (see note above)
...
//
// ...rest is spread onto the real <button>, which is how callers attach things
// like data-tour without this component needing to know about them.
...
// The primary button carries data-tour="modal-primary" so the tutorial can point
// at "confirm this" without knowing which dialog is open. It is on the confirm
// action specifically, never the cancel, so a tour step can only ever push a
// player forward through a dialog and never out of one.
  ...
  return c.jsx(Backdrop, { children: c.jsxs("div", {
    ...
    children: [
      ...
      c.jsxs("div", { style: { display: "flex", gap: 12, justifyContent: "flex-end", flexWrap: "wrap" }, children: [
        ...
        c.jsx(Button, { variant: primaryVariant, size: "sm", onClick: onPrimary, "data-tour": "modal-primary", style: { fontSize: 14 }, children: primaryLabel })
      ] })
    ]
  }) });
}
...
// ---------------------------------------------------------------------------
// Intensity scales with the stakes. Generated once via useMemo so the pieces
// don't re-randomize on every re-render mid-fall.
